...
export function queryRequiredElement<T extends Element>(id: string, ctor: new () => T): T | null {
  const el = document.getElementById(id);
  if (el === null) return null;
  if (el instanceof ctor) return el;
  console.warn(`[dom] queryRequiredElement: #${id} is not an instance of ${ctor.name}`);
  return null;
}
