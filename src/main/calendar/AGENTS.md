...
| `offline-cache.ts`                        | Encrypted `userData/calendar-cache.enc` — schema v1; owner-only modes via `secure-fs`; meetUrl via `validateMeetUrl`; rejects legacy/unversioned; filters ended events                                                                         |
| `auth/google-client-id.ts`                | `GOOGLE_OAUTH_CLIENT_ID`                                                                                                                                                                                                                      |
| `auth/google-token-store.ts`              | Encrypted `userData/calendar-auth/google.enc` (owner-only modes); **preserve ciphertext** on load failures                                                                                                                                    |
| `auth/google-sync-tokens.ts`              | Encrypted `userData/calendar-auth/google-sync.enc` (owner-only modes) — schema v1 `{version,tokens}` map of calendarId → opaque `nextSyncToken` (no event bodies)                                                                             |
...
| `providers/google-calendar.ts`            | Google API; live complete/partial; **incremental sync** + process-local index (prune outside poll window after incremental); extract join URLs from **raw** description before clean; **pagination-limit** (MAX_PAGES=50); cache write **complete only** |
...
- Use domain `extractMeetingUrl` / `cleanDescription`; extract from raw HTML descriptions before clean; never reimplement host allowlists ad hoc.
