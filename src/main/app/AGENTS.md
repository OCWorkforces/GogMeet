...
- `shutdownApp`: power cleanup → clear display horizon → force-destroy hide-cached **alert / settings / about / update** windows → graph stop (or free-function scheduler/watcher fallback if no graph) → unregister shortcuts → clear `activeGraph`.
- Both files are for `index.ts` only (plus tests).
- `initAutoUpdater()` runs last among non-critical init steps; no-ops when `!app.isPackaged` or portable. Otherwise configures install policy (`full` / feed-only) and schedules a ~5s quiet background check (log-only; no Restart dialog). Tray **Check for Updates…** calls `checkForUpdatesManual()` which presents `windows/update-window.ts` (aurora; checking → result; `beginUpdateDialogSession` each entry; dismiss-only Esc or multi-action buttons).
