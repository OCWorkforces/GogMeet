...
- `initAutoUpdater()` runs last among non-critical init steps; the module no-ops when `!app.isPackaged` or portable (`isPortableInstall`); otherwise auto-download + install-on-quit with a ~5s delayed first `checkForUpdates` (log-only errors).
