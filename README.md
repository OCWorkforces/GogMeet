...
- **Official releases** (`.github/workflows/release.yml`): a `main` push creates `v${package.json.version}` when missing. The `v*` tag job (excluding `-beta-` tags) requires `CSC_LINK`, `CSC_KEY_PASSWORD`, `APPLE_ID`, `APPLE_TEAM_ID`, and `APPLE_APP_SPECIFIC_PASSWORD`, runs `bun run package` once, verifies containers, writes `SHA256SUMS.txt`, and uploads arm64/x64 DMG/ZIP as **Latest**.
